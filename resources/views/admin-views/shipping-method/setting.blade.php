...
@section('content')
    <div class="content container-fluid">
                            ...
                            <div class="col-12 pl-8 text-capitalize">
                                <div class="row">
                                    <div class="form-check">
                                        <input onclick="shipping_responsibility(this.value);" class="form-check-input"
                                            type="radio" name="shipping_res" value="inhouse_shipping"
                                            id="inhouse_shipping" {{ $shippingMethod == 'inhouse_shipping' ? 'checked' : '' }}>
                                ...
                                <div class="row">
                                    <div class="form-check">
                                        <input onclick="shipping_responsibility(this.value);" class="form-check-input"
                                            type="radio" name="shipping_res" value="sellerwise_shipping"
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-12 text-capitalize"
                                ...
                                <label for=""
                                    id="for_inhouse_deliver">{{ \App\CPU\translate('for_inhouse_deliver') }}</label>
                                <select class="form-control text-capitalize" name="shippingCategory"
                                    onchange="shipping_type(this.value);" style="width: 100%">
                                    <option value="0" selected disabled>---{{ \App\CPU\translate('select') }}---
                                    </option>
                                    <option value="order_wise" {{ $shippingType == 'order_wise' ? 'selected' : '' }}>
                                        {{ \App\CPU\translate('order_wise') }} </option>
                                    <option value="category_wise" {{ $shippingType == 'category_wise' ? 'selected' : '' }}>
                                        {{ \App\CPU\translate('category_wise') }}</option>
                                    <option value="product_wise" {{ $shippingType == 'product_wise' ? 'selected' : '' }}>
                                        {{ \App\CPU\translate('product_wise') }}</option>
                                </select>
                            </div>
                            <div class="col-12 mt-2" id="product_wise_note">
                                <p class="m-2" style="color: red;">{{ \App\CPU\translate('note') }}:
                                    {{ \App\CPU\translate("Please_make_sure_all_the product's_delivery_charges_are_up_to_date.") }}
                                </p>
                            </div>
                        </div>
                    </div>
        ...
        @php($categories = App\Model\Category::get())
        ...
        <div id="order_wise_shipping">
            ...
            <div class="row" style="margin-top: 20px">
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header text-capitalize">
                            <h4>{{ \App\CPU\translate('order_wise_shipping_method') }} <span
                                    style="color: red;">({{ $shipping_methods->count() }})</span></h4>
                        </div>
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-bordered" width="100%" cellspacing="0"
                                    ...
                                    <thead>
                                        <tr>
                                            ...
                                            <th scope="col" style="width: 100px">{{ \App\CPU\translate('action') }}
                                            </th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @foreach ($shipping_methods as $k => $method)
                                            <tr>
                                                ...
                                                <td>
                                                    <label class="switch">
                                                        <input type="checkbox" class="status" id="{{ $method['id'] }}"
                                                ...
                                                <td>
                                                    ...
                                                    <div class="d-flex">
                                                        ...
                                                        <a title="{{ \App\CPU\translate('delete') }}"
                                                            ...
                                                            style="cursor: pointer;" id="{{ $method['id'] }}">
        ...
        function shipping_responsibility(val) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    toastr.success(
                        "{{ \App\CPU\translate('shipping_responsibility_updated_successfully!!') }}");
    ...
    <script>
        ...
        $(document).on('change', '.status', function() {
            ...
            $.ajax({
                ...
                success: function() {
                    toastr.success(
                        '{{ \App\CPU\translate('order wise shipping method Status updated successfully') }}'
                        );
                }
            });
        });
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        ...
                        success: function() {
                            toastr.success(
                                '{{ \App\CPU\translate('Order Wise Shipping Method deleted successfully') }}'
                                );
                            location.reload();
                        }
                    });
